      ...
      <div v-if="page == 'yourevents'">
        <v-card v-for="(even, i) in events" :key="i" style="margin-bottom: 20px;">
          <v-container fluid grid-list-lg>
            <v-layout row wrap>
              <v-flex xs4>
                <v-img :src="even.imageUrl" aspect-ratio="0.667" height="274" contain></v-img>
              </v-flex>
              <v-flex xs4>
                <v-card-title>
                  <h3>{{even.name}}</h3>
              ...
              <v-flex xs4>
                <div style="margin: 50px;">
                  <v-btn @click="toEventReport(even._id)" large dark color="blue">Event Report</v-btn>
                  <br>
  ...
  methods: {
    ...
    async onDecode(decodedString) {
      try {
        ...
        this.$swal("Thank You For Attending to Our Event", "", "success");
        console.log("berhasil memasukan", userId, eventId);
      } catch (error) {
        console.log(error.response.data.message);
        this.$swal(`You Already Attended to Our Event`, "", "error");
      }
    },
    toEventDetail(id) {
